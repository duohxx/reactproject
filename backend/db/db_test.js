 ...
 */
  ...
  // 1.2. 连接指定数据库(URL只有数据库是变化的)
  mongoose.connect('mongodb://127.0.0.1:27017/gzhipin_test')
  // 1.3. 获取连接对象
  ...
  function testSave() {
    ...
    const userModel1 = new UserModel({username: 'Tom', password: md5('456'), type: 'dashen'})
    // 调用save()保存
  ...
  function testFind() {
    ...
    UserModel.find({username: 'Bob'}, function (error, users) {
